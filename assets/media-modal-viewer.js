...
class VideoModalViewer extends HTMLElement {
  constructor() {
    super();
    this.initModal();
  }
  ...
  openModal(modal, modalBody, img) {
    const video_src = img.querySelector("source").getAttribute("src");
    const image_src = img.querySelector("img").getAttribute("src");
    console.log(video_src);
    modal.classList.remove("hidden");
    const imageHtml = `<video class="modal-video"  controls playsinline="playsinline" preload="metadata" aria-label="Potion beats" poster="${image_src}"><source src="${video_src}" type="video/mp4"><img src="${image_src}"></video>`;
    modalBody.innerHTML = imageHtml;

    const imageElement = modal.querySelector("video");
    setTimeout(() => {
      // imageElement.classList.remove("hover-animation-start");
      // imageElement.classList.add("hover-animation-end");
    }, 10);
...
customElements.define("video-modal-viewer", VideoModalViewer);
